...
When making a new offer if the sender is **NOT CERTAIN** that the offer will be completely matched or making a new offer which is **WORSE** than the _best_ one,
the pos should be calculate and passed with the method call.
If the sender is **NOT CERTAIN** where the offer should be placed, _0_ should be sent as value for _pos_.
That will trigger iteration over the side of the orderbook which might result in higher gas cost or a failed transaction ( out of gas )
