...
Calling this method to make a new offer might have three outcomes:
   - Offer is not matched and is put into the order book - sender becomes Maker
   - Offer is fully matched - sender becomes Taker
   - Offer is partially matched  - sender is both Maker and Taker. The remaining of the unfilled offer will be placed as a new order.
...
**_pos_** is the **OFFER ID** of the first offer that is higher in price than the new offer that the sender is making.
...
That way the sender is _taking_ an order.
There are other methods for a sender to take orders.
...
Used to cancel an offer with specifc if placed by the sender.
