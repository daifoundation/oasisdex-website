...
If _DAI_'s address is passed as a _pay_gem_ then the id of the best _bid_ will be returned.
...
Used to get the next worse offer in the sorted linked list. If the offer which _id_ is used in the method call ( current offer ) is an _ask_ one then a worse offer will have higher price.
...
### **Filling/taking orders ( Market Takers )**

One of the ways that we already have discussed in the `Placing Orders` section is to make a new offer that will fill completely an existing order.
That way the caller is _taking_ an order.
There are other methods for a caller to take orders.
...
It will transfers funds from caller to offer maker, and from market to caller
It is **NOT RECOMMENDED** to call this method directly. May end up filling an order that is not the optimal one.
...
### **Average Price fill-or-kill orders**
This API methods described below are dedicated to Takers and works by executing a special type of order called **"Average Price fill-or-kill order"**.
...
the transaction will revert. For example, the discontinued frontend oasis.direct used this, and it is now what Oasis Instant uses.
...
**Use case**: Given the _WETH/DAI_ Market. _Bids_ side of the order book looks like as follow:
 ...
 - **price**: 10 DAI, **amount**: 1.0 WETH, **total**: 10.0 DAI
   ...
   While order signing, someone else might sign similar order, the _makers_ of the first 3 orders
...
Methods that operate over ones offers.
