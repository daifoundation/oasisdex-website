...
If the current offer is a `bid`, then the offer returned will have higher price.
...
When creating new offer, funds are transffered to the contract's escrow ensuring that settlement is always possible.
...
`pos` is the **OFFER ID** of the first offer that has higher (or lower depending on wether it is `bid` or `ask` ) price than the new offer that the caller is making.
...
If you pass `pos=0` this means that the contract will have to search order list from the very begining. In the worst case, this can end up using a lot of gas or even cause tx revert.

Providing `pos` that is exactly correct is challening due to the various factors (ie. mempool frontrunning), but it's sufficient to provide `pos` close to the expected value. Matching engine will iterate from that place in a sorted list to find the correct location. The closer you are from the real position, the less amount of gas the contract will spend.

##### Rounding behaviour

The default rounding behaviour will match offers that differ in the price indicated by taker by 1 WEI. If this is not desirable `offer` function with additional parameter `rounding` set to `false` should be used instead.
...
This method IS NOT recommended and shouldn't be used. Such offer would not end up in the sorted list but would rather needed to be insterted by a keeper at a later date. There is no guarantee that this will ever happen.

#### `insert(uint, unint)`

Inserts an unsorted offer to the sorted list. This method should only be used if an offer was created by `offer(uint, ERC20, uint, ERC20)` method that does not insert the offer into a sorted list automatically.
...
Another method that the Taker can use is the `buy` method which fills a specific order (not necessarly the best one). This method can be used if the taker wants skip the matching (which will save gas) however, there is no best price guarantee, hence it should be used with caution.
...
This method is used to exchange ERC20 tokens, where the payAmt is sent to the matching market from the caller.
...
**Example**: Let's assume this bid side of the _WETH/DAI_ orderbook:
...
If the user wanted to sell **1 WETH** they would receive 15.2 DAI. This could be specific as `min_fill_amount` value, however as the orderbook might change before this order is mined (and executed), the user could specify the additional slippage and set `min_fill_amount` value to 14.44 DAI (5% slippage). This will improve the chance of the order being executed, hovewer it will increase the likelihood of being frontrun by market making bots.
...
This method is used to exchange ERC20 tokens. where the buyAmt is sent to the matching market from the caller.
